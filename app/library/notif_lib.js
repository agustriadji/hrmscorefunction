        ...
        async.waterfall([
            (next) => {
                ...
                (async function () {
                    ...
                    const _adminx =  await database.promise().query(`select employee_id from  ldap, role where ldap.role_id = role.role_id and (lower(role.role_name) like '%admin%' AND (lower(role.role_name) NOT LIKE '%administrator%')`);
